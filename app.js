...
const io = new Server(server, {
  cors: {
    origin: "https://tubular-kleicha-856a67.netlify.app",
    methods: ["GET", "POST"],
  },
});
...
io.on("connection", (socket) => {
  ...
  socket.on("send-Ref", async (data) => {
    ...
    try {
      const receiver = await userData.findOne({
        refrenceId: data.recieverRefrenceId,
      });

      const unreadMessages = await userMessage
        .find({
          $and: [
        ...
        .sort({ createdAt: 1 });
      io.to(roomName).emit("recievealluser-message", {
  ...
  socket.on("send-chstnotify", async (data) => {
    ...
    try {
      ...
      const unreadMessages = await userMessage
        ...
        .sort({ createdAt: 1 });
      io.to(roomName).emit("recievealluser-messageNotification", {
        messages: unreadMessages,
      });
    } catch (error) {
      console.error("Error fetching and emitting messages:", error);
  ...
  socket.on("messagejoin-room", async (data) => {
    const roomName = generateRoomName(
      data.senderRefrenceId,
      data.recieverRefrenceId
    );
    socket.join(roomName);
    console.log(`User joined room: ${roomName}`);

    try {
      const sender = await userData.findOne({
        refrenceId: data.senderRefrenceId,
      });
      const receiver = await userData.findOne({
        refrenceId: data.recieverRefrenceId,
      });
  ...
  socket.on("send-message", async (data) => {
    try {
      const roomName = generateRoomName(
        data.messageRoom.senderRefrenceId,
        data.messageRoom.recieverRefrenceId
      );
      const sender = await userData.findOne({
        refrenceId: data.messageRoom.senderRefrenceId,
      });
      const receiver = await userData.findOne({
        refrenceId: data.messageRoom.recieverRefrenceId,
      });
